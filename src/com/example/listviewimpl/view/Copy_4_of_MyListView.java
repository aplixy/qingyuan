...
public class Copy_4_of_MyListView extends AdapterView<Adapter> {
	...
	public Copy_4_of_MyListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public Copy_4_of_MyListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public Copy_4_of_MyListView(Context context) {
		super(context);
		init();
	}
	...
	private void fillListDown(int bottomEdge, float offset, int parentLeft) {
		...
		int lastItemPosition = mFirstItemPosition + getChildCount();
		while (bottomEdge + offset < getHeight() && lastItemPosition < mAdapter.getCount() - 1) {
			lastItemPosition++;
			cachedView = getCachedView();
			newBottomChild = mAdapter.getView(lastItemPosition, cachedView, this);
	...
	private void initList(int parentLeft) {
		...
		for (int BottomEdge = 0; BottomEdge < getHeight() && lastItemPosition < mAdapter.getCount(); lastItemPosition++) {
			View newBottomChild = mAdapter.getView(lastItemPosition, null, this);
	...
	private void layoutPositionItems(int parentLeft) {
		...
		if (mFirstItemPosition == 0 && top > 0){
			top = 0;
		}else if (mFirstItemPosition + getChildCount() == mAdapter.getCount() - 1){
	...
	// if (mLastItemPosition == mAdapter.getCount() && bottom < getHeight())
	// bottom = getHeight();
